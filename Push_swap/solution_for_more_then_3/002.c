...
int choose_move(num **list, int len)
{
	int i = 0;
	while (i < len)
	{
		printf("| %2d should go to %2d |\n",list[i]->value, list[i]->index);
...
int main(void)
{
	...
	while (i < len)
	{
		...
		list[index_end[index_start[i]]] = ft_calloc(1, sizeof(num));
		list[index_end[index_start[i]]]->index = i;
		list[index_end[index_start[i]]]->value = array1[index_end[index_start[i]]];
		i++;
	}
	printf("\n");
	choose_move(list, push_len);
}
