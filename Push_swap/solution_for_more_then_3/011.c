...
void cut_from_stack(stack *A, stack *B, int index_to_split_from)
{
	int i = 0;
	int j = 0;
	int sa = 0;
	int sb = 0;
	printf("\nindex to cut from : %d \nA lenght: %d\n", index_to_split_from, A->lenght);
	while (i < A->lenght - index_to_split_from)
	{
		if (A->array_of_nums[0].index >= index_to_split_from)
		{
			printf("\npush(%d) | i:%d \n", A->array_of_nums[0].value, i);
			push(A, B);
			moves++;
			i++;
		}
		else
		{
			...
			while (j < A->lenght)
			{
				if (A->array_of_nums[j].index >= index_to_split_from)
				{
					while (j > 0)
					{
						printf("r ");
				...
				if (A->array_of_nums[A->lenght - j - 1].index >= index_to_split_from)
				{
					while (j < A->lenght - 1)
					{
						printf("rr ");
...
int main(void)
{
	...
	int const_array[] = {0, 4, 11, 9, 5, 6, 7, 3, 10, 1, 26, 2};
